...
`compilation-mode-on-exit-sentinel'."
  (let* ((buffer (get-buffer "*compilation*"))
	...
	;; TODO: Need to figure out how to make a combined sentinel
	;; work
	;;
	;; (combined-sentinel
	;;  (lambda (process line)
	...
	(if (> (length sentinel-list) 1)
	    (message "TODO: need to combine the sentinels (total %d) , currently only applies the `compilation-mode-on-exit-sentinel`" (length sentinel-list)))
	(--each sentinel-list (setf (process-sentinel process) it))))))
